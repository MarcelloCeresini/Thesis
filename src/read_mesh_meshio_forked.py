...
COPIED from meshio repo: https://github.com/nschloe/meshio
"""
import re
...
def _skip_to(f, char):
    ...
    while c != char:
        c = f.read(1).decode()
        # tmp = f.tell()
        # following_char = f.read(1).decode()
        # if following_char == "\n":
        #     pass
        # else:
        #     f.seek(tmp)
...
def _read_points(f, line, first_point_index_overall, last_point_index):
    ...
    # (3010 (zone-id first-index last-index type dimension)
    ...
    if out.group(1) == "":
        # ASCII data
        pts = np.zeros((num_points, dim)) ### if dim > data, the other dimensions must be 0
        for k in range(num_points):
            # skip ahead to the first line with data
            line = ""
            while line.strip() == "":
                line = f.readline().decode()
                if closed_brackets_n := line.count(")") > 0: # could have brackets on the last line
                    line = line.replace(")", "")
    ...
    _skip_close(f, 2-closed_brackets_n)
    return pts, first_point_index_overall, last_point_index, zone_id, point_type
...
def _read_cells(f, line):
    ...
    if key == "mixed":
        ...
        # though. Skip for now.
        while len(cell_types) < num_cells:
            line = ""
            while line.strip() == "":
                line = f.readline().decode()
                if closed_brackets_n := line.count(")") > 0: # could have brackets on the last line
            ...
            dat = line.split()
            cell_types += dat
            if len(cell_types) > num_cells - 80:
                pass

        data = {"data": np.array(cell_types), "zone_id": zone_id}
    else:
        # read cell data
        if out.group(1) == "":
            # ASCII cells
...
def _read_faces(f, line):
    ...
    if out.group(1) == "":
        ...
        if key == "mixed":
            ...
            # read cell data
            data = np.zeros((num_cells, num_nodes_per_cell), dtype=int)
            adjacent_cells = np.zeros((num_cells, 2), dtype=int)
            for k in range(num_cells):
                line = ""
                while line.strip() == "":
                    line = f.readline().decode()
                ...
                adjacent_cells[k] = [int(d, 16) for d in dat[num_nodes_per_cell:]]

            
            data = {key: data}
            info_face = {
                "adj_cell": adjacent_cells, 
                "zone_id":zone_id,
                "bc_type":bc_type}
        
            # adjacent_cells = {key: adjacent_cells}
    else:
        # binary
        if out.group(1) == "20":
            dtype = np.int32
    ...
    with open_file(filename, "rb") as f:
        while True:
            ...
            if index == "0":
                ...
                # points
                pts, first_point_index_overall, last_point_index, zone_id, point_type = _read_points(
                    f, line, first_point_index_overall, last_point_index
                )

                if pts is not None:
                    points.append(pts)
                    info["elements"][zone_id] = {
                        "elem_type":"point", 
                        "point_type":point_type,
                    }
                    # info["zone_id_list"].append(zone_id)
                ...
                data, info_face = _read_faces(f, line)
                if info_face:
                    info["elements"][info_face["zone_id"]] = {
                        "elem_type":"face",
                        "adj_data":info_face["adj_cell"],
                        "bc_type":info_face["bc_type"],
                    }
                    info["zone_id_list_cellblocks"].append(info_face["zone_id"])
                for key in data:
                ...
                if obj:
                    dat = obj.group(0).split("(")[2].replace(")","").split()
                    zone_id = dat[0]
                    zone_type = dat[1]
                    zone_name = dat[2]
                    info["global"][zone_id] = {"zone_type":zone_type, "zone_name":zone_name}
